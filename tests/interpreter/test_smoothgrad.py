...
class TestSG(unittest.TestCase):
    ...
    def test_cv_sg(self):
        ...
        sg = it.SmoothGradInterpreter(paddle_model, use_cuda=False)
        exp = sg.interpret(img_path, visual=False)
    ...
    def test_cv_sg_class(self):
        np.random.seed(42)
        paddle_model = mobilenet_v2(pretrained=True)

        img_path = 'imgs/catdog.jpg'
        sg = it.SmoothGradInterpreter(paddle_model, use_cuda=False)
        exp = sg.interpret(img_path, labels=282, visual=False)
        result = np.array([exp.mean(), exp.std(), exp.min(), exp.max()])
    ...
    def test_cv_sg_multiple_inputs(self):
        np.random.seed(42)
        paddle_model = mobilenet_v2(pretrained=True)
