...
class TestGradCAm(unittest.TestCase):
    ...
    def test_cv_gradcam(self):
        paddle_model = mobilenet_v2(pretrained=True)

        img_path = 'imgs/catdog.jpg'
        gradcam = it.GradCAMInterpreter(paddle_model, use_cuda=False)
        exp = gradcam.interpret(img_path, 'features.18.2', visual=False)
    ...
    def test_cv_gradcam_layer(self):
        paddle_model = mobilenet_v2(pretrained=True)

        img_path = 'imgs/catdog.jpg'
        gradcam = it.GradCAMInterpreter(paddle_model, use_cuda=False)
        exp = gradcam.interpret(img_path, 'features.16.conv.3', visual=False)
        result = np.array([exp.mean(), exp.std(), exp.min(), exp.max(), *exp.shape])
    ...
    def test_cv_gradcam_layer_2(self):
        paddle_model = mobilenet_v2(pretrained=True)

        img_path = 'imgs/catdog.jpg'
        gradcam = it.GradCAMInterpreter(paddle_model, use_cuda=False)
        exp = gradcam.interpret(img_path, 'features.8.conv.3', visual=False)
        result = np.array([exp.mean(), exp.std(), exp.min(), exp.max(), *exp.shape])
    ...
    def test_cv_gradcam_multiple_inputs(self):
        paddle_model = mobilenet_v2(pretrained=True)
