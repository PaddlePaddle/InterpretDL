...
class TrainingDynamics():
    ...
    def save(self,logits,assigned_targets,label_flip=None,save_path=None):
        ...
        np.savez_compressed(os.path.join(save_path, 'training_dynamics.npz'), **{'label_flip': label_flip, 'labels': self.labels, 'td': self.training_dynamics})
        return self
...
class BHDFinterpreter(Interpreter):
    ...
    def interpret(self,
        ...
        """Call this function to rank samples' hardness.

        Args:
            load_path (_type_, optional): The filepath to save the processed image. 
                If None, the image will not be saved. Default: None
        ...
        dataset = paddle.io.TensorDataset([training_dynamics,paddle.zeros((50000,))])
        loader = paddle.io.DataLoader(dataset, batch_size=128, shuffle=False)        
        
        # try:
        paddle.Model(self.paddle_model).load(detector_model_path)
        # except:
        #     raise Exception('%s is not a right path.' % detector_model_path)

        predictions=[]
        with paddle.no_grad():
            for batch_id, data in enumerate(loader()):
                x_data = data[0]
                predicts = self.paddle_model(x_data).cpu().detach().numpy()
